...
def saveMatrix(matrixA, matrixB, matrixin, filename):
    if os.path.exists(filename): 
        os.remove(filename)
    else:   
        print("New file created: ",filename)
    f = open(filename, "w")
    for month in (matrixin):
        f.write("\t")
        f.write(str(month))
    f.write("\n")
...
def main():
    ...
    #Create dense matrix
    matrixin = [n]
    matrixA = createRandomMatrix(int(n/2),100)
    ...
    if (args.sparsity):
        ...
        #print(matrixA)
    saveMatrix(matrixA, matrixB, matrixin, args.dump)
